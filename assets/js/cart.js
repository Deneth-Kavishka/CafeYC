...
class CafeYCCart {
    ...
    saveCart() {
        localStorage.setItem('cafeyc_cart', JSON.stringify(this.cart));
        this.updateCartUI();
    }
    ...
    updateRowTotal(productId, quantity) {
        const priceElement = document.querySelector(`[data-price-for="${productId}"]`);
        const totalElement = document.querySelector(`[data-total-for="${productId}"]`);
